...
void convert_data(params* input){
  ...
  //  #pragma omp parallel for
    ...
    for(h=1;h<=degree;h++){
        ...
        //scrivi_colonne(tuple, n, coldest, h, j, input);

        popola(tuple, coldest, h, n, input,j);

        coldest+=tuple; //andiamo a scrivere la colonna coldest successiva di xast
    }
