...
; -------------------------------------------------  num1=num1* ->x[(oss ... oss+p-1) + x_i*n]
		...
		MOVAPS		XMM1,[ebx + eax*dim] 		; XMM1 = input->x[(oss ... oss+p-1) + x_i*n];
		MULPS		XMM0, XMM1					; num1=num1* input->x[(oss ... oss+p-1) + x_i*n];

		MOVAPS		XMM3,[ebx + 16 + eax*dim] 	; XMM1 = input->x[(oss+p ... oss+2p-1) + x_i*n];
		MULPS		XMM2, XMM3					; num1=num1* input->x[(oss+p ... oss+2p-1) + x_i*n];
		
		MOVAPS		XMM5,[ebx + 32 + eax*dim] 	; XMM1 = input->x[(oss+2p ... oss+3p-1) + x_i*n];
		MULPS		XMM4, XMM5					; num1=num1* input->x[(oss+2p... oss+3p-1) + x_i*n];

		MOVAPS		XMM7,[ebx + 48 + eax*dim] 	; XMM1 = input->x[(oss+3p ... oss+4p-1) + x_i*n];
...
;------------------------------------------------  input->xast[(oss ... oss+p-1)+coldest*n]=num1
		...
		MOVAPS 		[eax+ebx*dim], XMM0			;x_ast[oss*dim + coldest*n*dim] = xmm0
		MOVAPS 		[eax+16+ebx*dim], XMM2		;x_ast[oss*dim + coldest*n*dim] = xmm2
		MOVAPS 		[eax+32+ebx*dim], XMM4		;x_ast[oss*dim + coldest*n*dim] = xmm4	
		MOVAPS 		[eax+48+ebx*dim], XMM6		;x_ast[oss*dim + coldest*n*dim] = xmm6
		add			edi, 16
		mov			eax, ecx					;eax=n
		sub 		eax, edi					;n-oss
		cmp			eax, 16
...
sommaVettori:
			...
			for1:	CMP		EDI,EDX
				...
				MOVAPS	XMM0,[EAX+EDI]
				MOVAPS	XMM1,[EAX+EDI+16]
				MOVAPS	XMM2,[EAX+EDI+32]
				MOVAPS	XMM3,[EAX+EDI+48]
				
				MOVAPS	XMM4,[EBX+EDI]
				MOVAPS	XMM5,[EBX+EDI+16]
				...
				MOVAPS	[EAX+EDI+32],XMM2
				MOVAPS	[EAX+EDI+48],XMM3
...
calcoloTheta:
				...
				for5:		CMP		ESI,ECX
						...
						MOVAPS	XMM1,[EAX+ESI]		;DaSottrarre[h]
						MOVAPS	XMM2,[EAX+ESI+16]	
						MOVAPS	XMM3,[EAX+ESI+32]	
						...
						MOVAPS	[EBX+ESI],XMM4
						MOVAPS	[EBX+ESI+16],XMM5
						MOVAPS	[EBX+ESI+32],XMM6
