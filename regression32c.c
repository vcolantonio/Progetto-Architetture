...
extern void popola1(int tuple, int n, int coldest, int h, int d, int* j, int col, MATRIX x, MATRIX xast);

void convert_data(params* input){

    int d,n,degree,h,tuple,oss;
	...
	j= (int*) malloc (1*sizeof(int)); //inizializzo J fittizia
    ...
    for(h=1;h<=degree;h++){
    	...
    	tuple=vettore_combinazioni[h]; //corrisponderà al numero di tuple in J
    	j= (int*) realloc (j, (tuple*h)*sizeof(int)); //h= dimensione della singola tupla
		...
		popola1(tuple, n, coldest, h, d, j, col, input->x, input->xast);
		coldest+=tuple;
		
    }
	
	
	free(j); free(vettore_combinazioni);
	input-> theta= (VECTOR) _mm_malloc(col*sizeof(type),16);
}//convert_data

extern void prodottoScalare(type*, VECTOR, MATRIX, int, int);
...
extern void moltiplicazionePerScalare(VECTOR vett_ris, MATRIX x, int ipersize, int size, type scalare);
/*void moltiplicazionePerScalare(VECTOR vett_ris, MATRIX x, int i, int size, type scalare){

	for(int k=0; k<size; k++){
		vett_ris[k]= scalare * x[i*size + k];
	}

}*/

extern void quadratoVettore(VECTOR g, int rigapersize, MATRIX G, int size);
...
extern void faiSommatoria(VECTOR daSottrarre, int size, VECTOR g, MATRIX G, int rigapersize, type epsilon);
...
extern void calcoloTheta(VECTOR daSottrarre, VECTOR theta, type eta_fratto_v, int size);
...
void sgd(params* input){
	...
	//vettore temporaneo
    daSottrarre= (VECTOR) _mm_malloc(size*sizeof(type),16);
	...
	it = 0;
  	if(adagrad){
	    type epsilon = 1E-8;
	    int riga = 0;
	    VECTOR g=(VECTOR) _mm_malloc(size*sizeof(type),16); //vettore gradiente
	    MATRIX G =(MATRIX) malloc(size*k*sizeof(type));

	    for(int m = 0; m<k*size; m++)
	        G[m]=0; //inizializzazione G
	      ...
	      for(i=0; i<n;){
	        //calcolo del minimo
	          ...
	          for(j=i; j<i+v; j++){
	            ...
	            //calcolo riga gj
	            int molt_jsize=j*size;
	            prodottoScalare(&cost_molt, theta, xast, molt_jsize, size);
	            cost_molt= cost_molt - y[j];
	            moltiplicazionePerScalare(g, xast, molt_jsize, size, cost_molt);
	          ...
	          calcoloTheta(daSottrarre, theta, eta/v, size);
	          /*for(int h =0; h<size; h++) {
	            daSottrarre[h] *= eta/v;
    ...
    VECTOR temp;
    temp= (VECTOR) _mm_malloc(size*sizeof(type),16);
    ...
    while(it<iter){
    	for(i=0; i<n; ){
    		...
    		//inizializzo vettori temporanei
            azzeramentoDaSottrarre(daSottrarre, size);
    		/*for(int index=0;index<size;index++){
          		daSottrarre[index]=0; //temp[index]=0;
        	}*/
