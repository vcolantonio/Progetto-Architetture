...
void convert_data(params* input){
  ...
  //  int* j;
    ...
    for(h=1;h<=degree;h++){
		...
		popola1(tuple, n, vettore_col[h], h, d, j, col, input->x, input->xast);
		free(j);
...
void sgd(params* input){
	...
	it = 0;
  	if(adagrad){
	    type epsilon = 1E-8;
	    int riga = 0;
	    VECTOR g=(VECTOR) malloc(size*sizeof(type)); //vettore gradiente
    ...
    VECTOR temp;
    temp= (VECTOR) malloc(size*sizeof(type));
...
int main(int argc, char** argv) {
    ...
    if(!input->silent)
        printf("Conversion time = %.4f secs\n", time);
    else
        printf("%.3f\n", time);
